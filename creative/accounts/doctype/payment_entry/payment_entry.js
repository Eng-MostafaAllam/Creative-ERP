...
{% include "creative/public/js/controllers/accounts.js" %}
...
frappe.ui.form.on('Payment Entry', {
	...
	party_type: function(frm) {
		...
		frm.set_query("party", function() {
			if(frm.doc.party_type == 'Employee'){
				return {
					query: "creative.controllers.queries.employee_query"
				}
			}
			else if(frm.doc.party_type == 'Customer'){
				return {
					query: "creative.controllers.queries.customer_query"
				}
			}
		});
	...
	party: function(frm) {
		...
		if(frm.doc.payment_type && frm.doc.party_type && frm.doc.party && frm.doc.company) {
			...
			return frappe.call({
				method: "creative.accounts.doctype.payment_entry.payment_entry.get_party_details",
				args: {
					company: frm.doc.company,
					party_type: frm.doc.party_type,
					party: frm.doc.party,
	...
	set_account_currency_and_balance: function(frm, account, currency_field,
		...
		if (frm.doc.posting_date && account) {
			frappe.call({
				method: "creative.accounts.doctype.payment_entry.payment_entry.get_account_details",
				args: {
					"account": account,
					"date": frm.doc.posting_date,
					"cost_center": frm.doc.cost_center
	...
	source_exchange_rate: function(frm) {
		...
		frm.set_df_property("source_exchange_rate", "read_only", creative.stale_rate_allowed() ? 0 : 1);
	},
	...
	target_exchange_rate: function(frm) {
		...
		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("target_exchange_rate", "read_only", creative.stale_rate_allowed() ? 0 : 1);
	},
	...
	get_outstanding_documents: function(frm, filters) {
		...
		return  frappe.call({
			method: 'creative.accounts.doctype.payment_entry.payment_entry.get_outstanding_reference_documents',
			args: {
				args:args
			},
			callback: function(r, rt) {
	...
	set_deductions_entry: function(frm, account) {
		if(frm.doc.difference_amount) {
			frappe.call({
				method: "creative.accounts.doctype.payment_entry.payment_entry.get_company_defaults",
				args: {
					company: frm.doc.company
				},
				callback: function(r, rt) {
	...
	bank_account: function(frm) {
		...
		if (frm.doc.bank_account && in_list(['Pay', 'Receive'], frm.doc.payment_type)) {
			frappe.call({
				method: "creative.accounts.doctype.bank_account.bank_account.get_bank_account_details",
				args: {
...
frappe.ui.form.on('Payment Entry Reference', {
	...
	reference_name: function(frm, cdt, cdn) {
		...
		if (row.reference_name && row.reference_doctype) {
			return frappe.call({
				method: "creative.accounts.doctype.payment_entry.payment_entry.get_reference_details",
				args: {
...
frappe.ui.form.on('Payment Entry', {
	cost_center: function(frm){
		if (frm.doc.posting_date && (frm.doc.paid_from||frm.doc.paid_to)) {
			return frappe.call({
				method: "creative.accounts.doctype.payment_entry.payment_entry.get_party_and_account_balance",
				args: {
					company: frm.doc.company,
					date: frm.doc.posting_date,
