...
from creative.accounts.general_ledger import (
	get_round_off_account_and_cost_center,
	make_gl_entries,
	make_reverse_gl_entries,
	merge_similar_entries,
)
from creative.accounts.party import get_due_date, get_party_account
from creative.accounts.utils import get_account_currency, get_fiscal_year
from creative.assets.doctype.asset.asset import get_asset_account, is_cwip_accounting_enabled
from creative.assets.doctype.asset_category.asset_category import get_asset_category_account
from creative.buying.utils import check_on_hold_or_closed_status
from creative.controllers.accounts_controller import validate_account_head
from creative.controllers.buying_controller import BuyingController
from creative.stock import get_warehouse_account_map
...
class PurchaseInvoice(BuyingController):
	...
	def on_submit(self):
		...
		if self.update_stock == 1:
			...
			from creative.stock.doctype.serial_no.serial_no import update_serial_nos_after_submit
	...
	def get_gl_entries(self, warehouse_account=None):
		self.auto_accounting_for_stock = creative.is_perpetual_inventory_enabled(self.company)
		...
		for item in self.get("items"):
			if item.is_fixed_asset:
				...
				if not self.update_stock:
					...
					if item.item_tax_amount and not cint(creative.is_perpetual_inventory_enabled(self.company)):
						asset_eiiav_currency = get_account_currency(eiiav_account)
						gl_entries.append(
							self.get_gl_dict(
								{
	...
	def update_billing_status_in_pr(self, update_modified=True):
		...
		for pr in set(updated_pr):
			from creative.stock.doctype.purchase_receipt.purchase_receipt import update_billing_percentage
