		...
		if (frm.doc.status !== "Closed") {
			...
			frm.add_custom_button(__("Task"), function() {
				frappe.model.open_mapped_doc({
					method: "creative.support.doctype.issue.issue.make_task",
					frm: frm
				});
			}, __("Create"));
	...
	reset_service_level_agreement: function(frm) {
		let reset_sla = new frappe.ui.Dialog({
			...
			primary_action: (values) => {
				...
				frappe.call("creative.support.doctype.service_level_agreement.service_level_agreement.reset_service_level_agreement", {
