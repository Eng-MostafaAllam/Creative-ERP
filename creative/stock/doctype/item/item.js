...
frappe.ui.form.on("Item", {
	...
	onload: function(frm) {
		creative.item.setup_queries(frm);
	...
	refresh: function(frm) {
		...
		if (frm.doc.has_variants) {
			...
			if(frm.doc.variant_based_on==="Item Attribute") {
				frm.add_custom_button(__("Single Variant"), function() {
					creative.item.show_single_variant_dialog(frm);
				}, __('Create'));
				frm.add_custom_button(__("Multiple Variants"), function() {
					creative.item.show_multiple_variants_dialog(frm);
				}, __('Create'));
			} else {
				frm.add_custom_button(__("Variant"), function() {
					creative.item.show_modal_for_manufacturers(frm);
				}, __('Create'));
			}
		...
		if (!frm.doc.published_in_website) {
			frm.add_custom_button(__("Publish in Website"), function() {
				frappe.call({
					method: "creative.e_commerce.doctype.website_item.website_item.make_website_item",
					args: {doc: frm.doc},
					freeze: true,
		...
		creative.item.edit_prices_button(frm);
		creative.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			creative.item.make_dashboard(frm);
		}
	...
	validate: function(frm){
		creative.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function(frm) {
		...
		frappe.call({
			method: "creative.stock.doctype.item.item.get_asset_naming_series",
			callback: function(r) {
...
$.extend(creative.item, {
	setup_queries: function(frm) {
		...
		frm.fields_dict.customer_items.grid.get_field("customer_name").get_query = function(doc, cdt, cdn) {
			return { query: "creative.controllers.queries.customer_query" }
		}
	...
	make_dashboard: function(frm) {
		...
		if (frm.doc.is_stock_item) {
			frappe.require('item-dashboard.bundle.js', function() {
				...
				creative.item.item_dashboard = new creative.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: 'creative.stock.dashboard.item_dashboard.get_data',
					template: 'item_dashboard_list'
				});
				creative.item.item_dashboard.refresh();
			});
		}
	},
	...
	show_modal_for_manufacturers: function(frm) {
		...
		dialog.set_primary_action(__('Create'), function() {
			...
			frappe.call({
				method: "creative.controllers.item_variant.get_variant",
				args: data,
				callback: function(r) {
	...
	show_multiple_variants_dialog: function(frm) {
		...
		function make_and_show_dialog(fields) {
			...
			me.multiple_variant_dialog.set_primary_action(__('Create Variants'), () => {
				...
				frappe.call({
					method: "creative.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						"item": frm.doc.name,
			...
			frappe.call({
				...
				callback: function(r) {
					...
					if (r.message) {
						...
						d.hide();
						frappe.call({
							method: "creative.controllers.item_variant.create_variant",
							args: {
								"item": frm.doc.name,
								"args": d.get_values()
							},
		...
		$.each(d.fields_dict, function(i, field) {
			...
			field.$input
				.on('input', function(e) {
					...
					frappe.call({
						method: "creative.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
frappe.ui.form.on("UOM Conversion Detail", {
	uom: function(frm, cdt, cdn) {
		...
		if (row.uom) {
			frappe.call({
				method: "creative.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					"uom": row.uom,
...
frappe.tour['Item'] = [
	...
	},
	{
		...
		description: __("If you are maintaining stock of this Item in your Inventory, creative will make a stock ledger entry for each transaction of this item.")
	},
	{
