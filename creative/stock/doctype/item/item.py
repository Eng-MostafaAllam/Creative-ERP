...
from creative.setup.doctype.item_group.item_group import invalidate_cache_for
from creative.stock.doctype.item_default.item_default import ItemDefault
...
class Item(Document):
			...
			item_price = frappe.get_doc(
				{
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": creative.get_default_currency(),
			...
			for item_barcode in self.barcodes:
				...
				if item_barcode.barcode:
					...
					if item_barcode.barcode_type:
						barcode_type = convert_creative_to_barcodenumber(
	...
	def recalculate_bin_qty(self, new_name):
		from creative.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"creative.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
...
def convert_creative_to_barcodenumber(creative_number, barcode):
	if creative_number == "EAN":
		ean_type = {
			8: "EAN8",
			13: "EAN13",
		}
	...
	if creative_number in convert:
		return convert[creative_number]
...
def invalidate_item_variants_cache_for_website(doc):
	...
	from creative.e_commerce.variant_selector.item_variants_cache import ItemVariantsCacheManager
...
def get_asset_naming_series():
	from creative.assets.doctype.asset.asset import get_asset_naming_series
