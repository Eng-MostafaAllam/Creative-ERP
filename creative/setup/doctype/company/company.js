...
frappe.provide("creative.company");
...
frappe.ui.form.on("Company", {
	...
	setup: function(frm) {
		creative.company.setup_queries(frm);
	...
	refresh: function(frm) {
		...
		creative.company.set_chart_of_accounts_options(frm.doc);
	},
	...
	delete_company_transactions: function(frm) {
		frappe.verify_password(function() {
			...
			function(data) {
				...
				frappe.call({
					method: "creative.setup.doctype.company.company.create_transaction_deletion_request",
					args: {
...
creative.company.set_chart_of_accounts_options = function(doc) {
	var selected_value = doc.chart_of_accounts;
	if(doc.country) {
		return frappe.call({
			method: "creative.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				"country": doc.country,
				"with_standard": true
			},
...
creative.company.setup_queries = function(frm) {
	$.each([
		...
		creative.company.set_custom_query(frm, v);
	});
...
creative.company.set_custom_query = function(frm, v) {
	var filters = {
