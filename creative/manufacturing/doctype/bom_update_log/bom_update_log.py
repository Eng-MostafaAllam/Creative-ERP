...
from creative.manufacturing.doctype.bom_update_log.bom_updation_utils import (
	get_leaf_boms,
...
class BOMUpdateLog(Document):
	...
	def on_submit(self):
		if self.update_type == "Replace BOM":
			...
			frappe.enqueue(
				method="creative.manufacturing.doctype.bom_update_log.bom_update_log.run_replace_bom_job",
				doc=self,
				boms=boms,
				timeout=40000,
			...
			frappe.enqueue(
				method="creative.manufacturing.doctype.bom_update_log.bom_update_log.process_boms_cost_level_wise",
				update_doc=self,
				now=frappe.flags.in_test,
			)
...
def queue_bom_cost_jobs(
	...
	while current_boms_list:
		...
		frappe.enqueue(
			method="creative.manufacturing.doctype.bom_update_log.bom_updation_utils.update_cost_in_level",
			doc=update_doc,
