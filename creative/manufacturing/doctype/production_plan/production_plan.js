...
frappe.ui.form.on('Production Plan', {
		...
		const projected_qty_formula = ` <table class="table table-bordered" style="background-color: var(--scrollbar-track-color);">
			<tr><td style="padding-left:25px">
				...
				<h3 style="text-decoration: underline;">
					<a href = "https://creative.com/docs/user/manual/en/stock/projected-quantity">
	...
	get_items_for_material_requests(frm, warehouses) {
		...
		frappe.call({
			method: "creative.manufacturing.doctype.production_plan.production_plan.get_items_for_material_requests",
			freeze: true,
			args: {
				doc: frm.doc,
	...
	download_materials_required(frm) {
		...
		frappe.prompt(fields, (row) => {
			let get_template_url = 'creative.manufacturing.doctype.production_plan.production_plan.download_raw_materials';
...
frappe.ui.form.on("Production Plan Item", {
	item_code(frm, cdt, cdn) {
		...
		if (row.item_code) {
			frappe.call({
				method: "creative.manufacturing.doctype.production_plan.production_plan.get_item_data",
				args: {
...
frappe.ui.form.on("Material Request Plan Item", {
	warehouse(frm, cdt, cdn) {
		...
		if (row.warehouse && row.item_code && frm.doc.company) {
			frappe.call({
				method: "creative.manufacturing.doctype.production_plan.production_plan.get_bin_details",
				args: {
					row: row,
					company: frm.doc.company,
...
frappe.ui.form.on("Production Plan Sales Order", {
	sales_order(frm, cdt, cdn) {
		...
		frappe.call({
			method: "creative.manufacturing.doctype.production_plan.production_plan.get_so_details",
			args: { sales_order },
			callback(r) {
