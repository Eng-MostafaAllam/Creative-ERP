frappe.ui.form.on("Communication", {
	...
	make_lead_from_communication: (frm) => {
		return frappe.call({
			method: "creative.crm.doctype.lead.lead.make_lead_from_communication",
			args: {
	...
	make_issue_from_communication: (frm) => {
		return frappe.call({
			method: "creative.support.doctype.issue.issue.make_issue_from_communication",
			args: {
				communication: frm.doc.name
			},
			freeze: true,
	...
	make_opportunity_from_communication: (frm) => {
		...
		frappe.prompt(fields, data => {
			frappe.call({
				method: "creative.crm.doctype.opportunity.opportunity.make_opportunity_from_communication",
				args: {
					communication: frm.doc.name,
